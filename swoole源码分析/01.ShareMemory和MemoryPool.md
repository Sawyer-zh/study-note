...
接下来看对应的四个处理函数，从名字上可以区分，mmap代表使用**内存映射文件的共享内存**，sysv代表使用**shm系列函数的共享内存**。这四个函数的实现写在ShareMemory.c文件中。这里重点解析 swShareMemory_mmap_create 方法和swShareMemory_sysv_create 方法。
...
```
使用这个宏代表使用mmap生成的映射区不与任何文件关联。我的理解是：此处创建一个仅存在与内存中的虚拟文件，用来存放需要共享的内容。
